...
python3 zap-api-scan.py -t http://backend:8080/v3/api-docs -f openapi -z "auth.header=Origin:\ http://frontend:80" -z "auth.header=Authorization:\ Bearer\ $TOKEN" -r report/api-report-auth.html -J report/api-report-auth.json -d
