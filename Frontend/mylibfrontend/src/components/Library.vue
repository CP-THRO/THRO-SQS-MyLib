  ...
  <div>
    ...
    >
      ...
      <template #individualRating="{ book }">
        {{book.individualRating ? `${book.averageRating} / 5` : 'Not rated yet'}}
      </template>
...
import {defineComponent, ref, computed, onMounted, onActivated, watch, onBeforeMount} from 'vue';
...
export default defineComponent({
  ...
  setup() {
    ...
    const loadBooks = async () => {
      ...
      try {
        books.value = await apiService.getLibrary(startIndex, pageSize.value);
      } catch (e: any) {
